...
namespace Mmu.Sms.WpfUI.Infrastructure.Wpf.DependencyProperties
{
    public abstract class EventCommandBinding
    {
        ...
        private static RoutedEvent GetRoutedEvent(UIElement uiElement)
        {
            ...
            var allRoutedEvents = typeof(UIElement).GetFields(BindingFlags.Static | BindingFlags.Public).Where(f => typeof(RoutedEvent).IsAssignableFrom(f.FieldType));
        ...
        private static void OnEvent(object sender, RoutedEventArgs e)
        {
            ...
            var cmd = uiElement?.GetValue(CommandProperty) as ICommand;
